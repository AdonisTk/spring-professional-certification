...
public class ArithmeticOperator {
	...
	@Value("#{19 + 1}") // Will inject 20.0
	private double add; 
	
	@Value("#{'string1 ' + 'string2'}") // Will inject "string1 string2"
	private String addString; 
	 
	@Value("#{20 - 1}") // Will inject 19.0
	private double subtract;
	 
	@Value("#{10 * 2}") // Will inject 20.0
	private double multiply;
	 
	@Value("#{36 / 2}") // Will inject 19.0
	private double divide;
	 
	@Value("#{36 div 2}") // Will inject 18.0, the same as for / operator
	private double divideAlphabetic; 
	 
	@Value("#{37 % 10}") // Will inject 7.0
	private double modulo;
	 
	@Value("#{37 mod 10}") // Will inject 7.0, the same as for % operator
	private double moduloAlphabetic; 
	 
	@Value("#{2 ^ 9}") // Will inject 512.0
	private double powerOf;
	 
	@Value("#{(2 + 2) * 2 + 9}") // Will inject 17.0
